...
def edit_matchs(match_id):
    ...
    if request.method == "GET":
        ...
        match_dict["date_match_local"] = (
            match_dict["date_match"][:16].replace(" ", "T")
...
def setup_staff(token):
    ...
    if not re_match(r"^[a-zA-Z0-9_]+$", username):
        return render_template(
            "admin/setup_staff.html", error="Le nom d'utilisateur est invalide."
        )

    if len(prenom) > 20 or len(nom) > 20 or len(username) > 20:
        return render_template(
            "admin/setup_staff.html", error="Certains champs sont trop longs !"
        )

    if len(mdp) < 8:
        return render_template(
            "admin/setup_staff.html", error="Le mot de passe est trop court !"
        )
    ...
    if get_user_by_email(email):
        return render_template(
            "admin/setup_staff.html", error="Cet email est déjà utilisé !"
        )
    ...
    try:
        ...
        return render_template(
            "admin/setup_staff.html", error="Veuillez entrer un âge valide."
        )
...
def transaction_action():
    ...
    if action == "valider":
        if tx["type"] == "depot":
            success, msg = credit(
                user["username"], float(tx["montant_dec"]), message=True
            )
            if success:
        ...
        if tx["type"] == "depot":
            ...
            # Remboursement
            succes_cred, msg = credit(
                user["username"], float(tx["montant_dec"]), message=False
            )
            if not succes_cred:
        ...
        envoyer_push_notification(
            user["push_subscription"], "Retrait refusé", message_push
        )
        refus_notification(
            nom=user["prenom"],
            email=user["email"],
