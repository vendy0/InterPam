...
def user_transactions(user_id):
    try:
        ...
        return render_template("admin/users/transactions.html", transactions=transactions, user=user)
    except Exception as e:
...
def nouveau_match():
    ...
    match_id = ajouter_match(equipe_a, equipe_b, date_match.replace("T", " "), type_match=type_match)
...
def show_edit_matchs():
    ...
    return render_template("admin/matchs/liste_matchs.html", programmes=programmes, titre=titre, mode=mode)
...
def edit_matchs(match_id):
    ...
    if request.method == "GET":
        options = get_options_by_match_id(match_id)
        match_dict = dict(match_data)
        match_dict["date_match_local"] = match_dict["date_match"][:16].replace(" ", "T") if match_dict.get("date_match") else ""
...
def mailbox():
    ...
    for u in users_list:
        if titre:
            success, msg = envoyer_notification_general(u["prenom"], u["email"], titre, message, lien, text_button)
            if success:
                emails_envoyes += 1
            else:
                error_email = msg
        ...
        if message_popup and titre_popup and u.get("push_subscription"):
            success_p, msg_p = envoyer_push_notification(u["push_subscription"], titre_popup, message_popup, lien)
            if success_p:
...
def transaction_action():
    ...
    if action == "valider":
        ...
        if tx["type"] == "retrait":
            # Rembourser le montant brut débité au moment de la demande
            credit(user["username"], float(tx["montant_dec"]))
