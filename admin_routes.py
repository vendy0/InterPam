...
def user_transactions(user_id):
    """Affiche l'historique global (P2P + Admin)"""
    try:
        user = get_user_by_id(user_id)
        if not user:
            flash("Utilisateur introuvable.", "error")
...
def user_details(user_id):
    """Affiche l'historique SPÉCIFIQUE des interventions manuelles (Admin Log)"""
    user = get_user_by_id(user_id)
    # Fonction désormais disponible grâce au correctif dans user.py
