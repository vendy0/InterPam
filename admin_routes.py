...
def find_user():
	...
	if len(resultats) > 0:
		return render_template("admin/users.html", users=resultats, current_user = get_user_by_username(session['username']))
	else:
...
def staff():
	...
	if succes:
		...
		print(f"Lien généré : {lien}")  # Pour que tu puisses copier-coller le lien
		flash(f"Invitation générée ! (Simulée pour l'email {email})", "succes")
