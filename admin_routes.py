...
def edit_matchs(match_id):
    ...
    if request.method == "POST":
        donnees = request.form
        equipe_a = donnees.get("equipe_a")
        equipe_b = donnees.get("equipe_b")
        ...
        for i in range(len(option_ids)):
            ...
            if o_id == "0":
                ajouter_option(libelles[i], float(cotes[i]), categories[i], match_id)  #
            else:
                update_option_info(o_id, libelles[i], float(cotes[i]), categories[i])
