...
def placer_pari(parieur_id, match_id, mise_dec, gain_dec, date_pari, options_ids):
	"""Accepte mise et gain en Decimal"""
	try:
		...
		with sqlite3.connect(DB_NAME) as conn:
			...
			solde_actuel = cur.fetchone()[0]
			if solde_actuel < mise_c:
				return False, "Solde insuffisant"

			cur.execute("UPDATE parieurs SET solde = solde - ? WHERE id = ?", (mise_c, parieur_id))
			...
			for opt_id in options_ids:
				cur.execute("SELECT match_id FROM options WHERE id = ?", (opt_id,))
				m_id = cur.fetchone()[0]
				cur.execute(
...
def get_fiches(parieur_id):
	try:
		with sqlite3.connect(DB_NAME) as conn:
			...
			cur = conn.cursor()
			cur.execute(
				"SELECT * FROM paris WHERE parieur_id = ? ORDER BY date_pari DESC", (parieur_id,)
			)
			return cur.fetchall()
	except sqlite3.Error as e:
