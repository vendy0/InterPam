...
EXPORT_DIR = f"exports_{DATE_EXPORT}"
...
def export_table(cursor, table_name, folder):
    ...
    with open(filename, "w", newline="", encoding="utf-8-sig") as csvfile:
        writer = csv.writer(
            csvfile, delimiter=";"
        )  # Point-virgule pour Excel/Calc facile
...
def main_export():
    mes = "DÉBUT DES EXPORTS".center(50, "_")
    print(mes)
    ...
    print(f"📂 Dossier créé : {EXPORT_DIR_CSV}\n")
    ...
    try:
        ...
        for table in tables:
            export_table(cur, table, EXPORT_DIR_CSV)

        print(
            f"\n✨ Terminé ! Tes fichiers CSV sont dans le dossier '{EXPORT_DIR_CSV}'."
        )
