...
def traitementLogin():
    ...
    utilisateur = get_user_by_email(email_username.lower()) or get_user_by_username(email_username)
...
def traitementRegister():
    ...
    categorie_success, message_pend = check_pending_duplicates(
        username,
        email,
    )
    if categorie_success == "success":
        pass
    elif categorie_success == "error" and message_pend:
        flash(message_pend, "error")
        return render_template("auth.html", register=True)
    else:
...
#     if not email:
#         return "Erreur : Adresse email manquante.", 400

#     # ==========================================
#     # ICI : LOGIQUE DE BASE DE DONNÉES
#     # Tu dois modifier la ligne correspondant à cet utilisateur
#     # pour lui retirer le droit de recevoir des notifications.
#     #
#     # Exemple avec SQLAlchemy :
#     # user = User.query.filter_by(email=email).first()
#     # if user:
#     #     user.accepte_emails = False
#     #     db.session.commit()
#     # ==========================================

#     # Message de confirmation simple (tu pourras faire un vrai template HTML plus tard)
#     html_response = f"""
#     <html>
#         <body style="text-align: center; font-family: sans-serif; padding-top: 50px;">
#             <h2>Désinscription réussie</h2>
#             <p>L'adresse <b>{email}</b> a été retirée de notre liste d'envoi.</p>
#             <p>Vous ne recevrez plus de notifications de la part d'InterPam.</p>
...
def unsubscribe_route():
    ...
    <div style="text-align:center; margin-top:50px; font-family:sans-serif;">
        ...
        <p>Vous ne recevrez plus de notifications non-essentielles d'InterPam.</p>
...
def details_match(match_id):
    ...
    return render_template("details_match.html", match=match_trouve, user=user, categories=categories_dict)
