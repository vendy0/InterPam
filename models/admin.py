				...
				if users:
					for user in users:
						envoyer_push_notification(
							user["sub"],
							"Match ajouté",
							f"Le match {equipe_a} VS {equipe_b} vient d'être ajouté. Cliquer ici pour commencer à parier dès maintenant.",
						)
			except Exception as e:
...
def update_match_info(match_id, equipe_a, equipe_b, date_match, statut, type_match):
	...
	try:
		with get_db_connection() as conn:
			closed = conn.execute(
				"SELECT * FROM matchs WHERE id = ? AND statut <> ?",
				(
					match_id,
					statut,
				),
			...
			if users:
				for user in users:
					if match and statut == "fermé":
						envoyer_push_notification(
							user["sub"],
							"Match fermé",
							f"Le match {equipe_a} VS {equipe_b} est fermé à tous nouveaux paris.",
						)
			return True
	except sqlite3.Error as e:
...
def executer_settlement_match(match_id):
	...
	try:
		...
		for pari in paris_a_verifier:
			...
			# LA CAISSE GAGNE :
			# 1. Elle récupère la réservation (le gain qu'elle n'a pas eu à payer)
			# 2. Elle garde la mise (profit)
			if total > 0:
				montant_retour_caisse = gain_c + mise_c
				mouvement_caisse(montant_retour_caisse, "add")
			...
			# CAS 2 : Toutes les options sont annulées -> Ticket ANNULÉ (Remboursement)
				...
				# LA CAISSE RÉCUPÈRE LA RÉSERVATION (pas de profit, pas de perte)
				# Note: 'gain_c' ici est égal à la mise car recalculé par annuler_match_et_rembourser
				# Si le gain n'a pas été recalculé, il faut utiliser l'ancien gain potentiel original.
				# Supposons ici que gain_c est le montant "réservé" à l'origine.
				# SI annuler_match a modifié gain_potentiel, il faut faire attention.
				# LOGIQUE SIMPLE : On remet dans la caisse ce qu'on a enlevé au pari.
				mouvement_caisse(gain_c, "add")
				stats["annules"] += 1
			...
			# Cela signifie qu'il n'y a pas de perdants, pas d'attente, et ce n'est pas 100% annulé
				...
				# La CAISSE ne bouge pas. Le montant a déjà été déduit. 
