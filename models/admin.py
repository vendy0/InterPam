...
def fermer_match_officiellement(match_id):
	...
	try:
		with get_db_connection() as conn:
			conn.execute("UPDATE matchs SET statut = 'terminé' WHERE id = ?", (match_id,))
			cur = conn.execute(
		...
		with get_db_connection() as conn:
			...
			if data_changed:
				...
				if old["statut"] == "ouvert" and statut == "fermé":
					notification_msg = (
						f"Le match {equipe_a} VS {equipe_b} est désormais fermé aux paris."
					)
				elif old["statut"] == "fermé" and statut == "ouvert":
					notification_msg = f"Le match {equipe_a} VS {equipe_b} est de nouveau ouvert !"
				else:
					notification_msg = (
						f"Le statut du match {equipe_a} VS {equipe_b} est passé à : {statut}."
					)
			else:
			...
			if notification_msg:
				...
				for user in users:
					try:
						envoyer_push_notification(user["sub"], notification_title, notification_msg)
					except:
						continue
	...
	try:
		...
		for pari in paris_a_verifier:
			...
			if (res["perdus"] or 0) > 0:
				cur.execute("UPDATE paris SET statut = 'Perdu' WHERE id = ?", (pari["id"],))
				stats["perdants"] += 1
			elif (res["annulés"] or 0) == res["total"]:
				cur.execute("UPDATE paris SET statut = 'Annulé' WHERE id = ?", (pari["id"],))
				cur.execute(
				...
				cur.execute("UPDATE paris SET statut = 'Gagné' WHERE id = ?", (pari["id"],))
				cur.execute(
					"UPDATE parieurs SET solde = solde + ? WHERE id = ?",
					(pari["gain_potentiel"], pari["parieur_id"]),
				)
			...
			if ban:
				...
				if user["sub"]:
					envoyer_push_notification(
						user["sub"], "Compte suspendu", message or "Votre compte est suspendu."
					)
...
def mark_as_read(message_id):
	try:
		with get_db_connection() as conn:
			...
			conn.execute("UPDATE messagerie SET read = ? WHERE id = ?", (new_status, message_id))
			conn.commit()
			return True
	except sqlite3.Error:
		return False
