...
def envoyer_email(message_dict):
    # 1. Validation des données
    champs_obligatoires = ["nom", "email", "subject", "corps"]
    if not all(message_dict.get(cle) for cle in champs_obligatoires):
        return False, "Il manque des informations dans le dictionnaire !"

    # 2. Configuration (Utilisez des variables d'environnement en production !)
    ...
    msg["To"] = message_dict["email"]

    # On construit le contenu du texte
    contenu = f"Bonjour {message_dict['nom']},\n\n{message_dict['corps']}"
    ...
    try:
        ...
        # Note : Le print après le return ne sera jamais exécuté,
        # donc on logge avant de renvoyer True.
        print(f"Email envoyé avec succès à {message_dict['email']}")
        return True, "Email envoyé avec succès !"

    except Exception as e:
        error_msg = f"Erreur lors de l'envoi : {e}"
        print(error_msg)
