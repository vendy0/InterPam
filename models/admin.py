        ...
        with get_db_connection() as conn:
            ...
            for option in options_du_match:
                ...
                for pari in paris_affectes:
                    ...
                    # Recalcul du nouveau gain
                    if cote_annulee > 1:
                        nouveau_gain = int(ancien_gain / cote_annulee)
                    else:
                        nouveau_gain = ancien_gain
                    ...
                    # --- CORRECTION ICI ---
                    # On rend à la caisse la part de la réservation qui n'est plus nécessaire
                    difference = ancien_gain - nouveau_gain
                    #                     if difference > 0:
                    # On réutilise la fonction mouvement_caisse
                    #                         Attention: mouvement_caisse attend un montant décimal,
                    #                         mais ici on a des centimes. Il faut convertir ou faire l'update SQL direct.
                    #                         Option SQL direct pour rester dans la même transaction :
                    conn.execute(
    ...
    try:
        ...
        for pari in paris_a_verifier:
            ...
            # CAS 2 : Ticket ANNULÉ (Remboursement total)
                ...
                # il retourne simplement au user.
                # mouvement_caisse(depuis_centimes(gain_c), "add")
        ...
        return (
            True,
            f"Settlement terminé : {stats['gagnants']} gagnés, {stats['perdants']} perdus, {stats['annules']} annulés.",
        )
