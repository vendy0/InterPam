...
def email_active(email):
    try:
        with get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute("SELECT accepte_emails FROM parieurs WHERE email = ?", (email,))
            res = cur.fetchone()
            if res and res[0] == 0:
                print(f"🚫 Envoi annulé : {email} est désinscrit.")
                return False
            return True
    except:
        return False
...
def get_user_by_name(nom):
    ...
    try:
        with get_db_connection() as conn:
            cur = conn.execute("SELECT * FROM parieurs WHERE prenom LIKE ? OR nom LIKE ?", (nom, nom))
            rows = cur.fetchall()
...
def get_admin_transactions_by_user(user_id):
    try:
        with get_db_connection() as conn:
            ...
            rows = cur.fetchall()
            return [dict(row) for row in rows]  # Pas besoin de convertir le montant ici si tu le fais dans le template
    except sqlite3.Error as e:
