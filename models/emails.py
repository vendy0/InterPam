...
def welcome_email(nom, email, lien):
	...
	corps_texte = f"Bonjour {nom}, bienvenue sur InterPam ! Accédez à votre espace ici : {lien}"
...
def envoyer_notification_generale(nom, email, titre, message, lien=None, texte_bouton=None):
	...
	with open("templates/admin/emails/general_notification.html", "r", encoding="utf-8") as f:
		template_html = f.read()
	...
	html = j2_template.render(
		nom=nom,
		titre_entete=titre,
		message_principal=Markup(message),  # Permet d'interpréter le HTML dans le template
	...
	message_clean = re.sub("<[^<]+?>", "", message_clean)  # Supprime toutes les autres balises HTML
