...
def fermer_match_officiellement(match_id):
	...
	try:
		with sqlite3.connect(DB_NAME) as conn:
			cur = conn.cursor()
			cur.execute("UPDATE matchs SET statut = 'terminé' WHERE id = ?", (match_id,))
			conn.commit()
			return True
	except sqlite3.Error as e:
...
def ajouter_column():
	try:
		with sqlite3.connect(DB_NAME) as conn:
			cur = conn.cursor()
			cur.execute("ALTER TABLE parieurs ADD COLUMN push_subscription TEXT")
			conn.commit()
...
#         with sqlite3.connect(DB_NAME) as conn:
#             cur = conn.cursor()
#             x = cur.execute("PRAGMA table_info(parieurs);")
#             res = x.fetchall()
#             for r in res:
#                 print(r)
#             print("Ok")
